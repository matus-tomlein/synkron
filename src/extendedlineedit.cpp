...
void ExtendedLineEdit::resizeEvent(QResizeEvent *)
{
...
void ExtendedLineEdit::enterEvent(QEvent * event)
{
    if (!hasFocus()) { selectAll(); setFocus(); }
    event->accept();
}

void ExtendedLineEdit::setClearButtonVisible(const QString & text) {
